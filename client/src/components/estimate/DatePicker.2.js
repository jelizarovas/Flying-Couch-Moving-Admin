...
export default function DatePicker() {
  ...
  const handleCalendarClick = (day, { selected, disabled, outside }) => {
    ...
    // const isOutside = outside || undefined;
    // if (isOutside) return console.log("OUTSIDE");
  ...
  const handleWeekClick = (weekNumber, days) => {
    ...
    const selectedDays = days.filter(day => {
      return !Array.isArray(anyMatches(day)) || !anyMatches(day).length
